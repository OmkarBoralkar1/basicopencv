...
while True:
    ...
    # Initialize frame_filter with the original frame
    frame_filter = frame.copy()

    # If a filter has been selected, apply it to the frame
    if filter_selected:
        frame_filter = filter_options[filter_selected](frame)
    ...
    if not filters_drawn:
        for name, rect in filter_rectangles.items():
            cv2.rectangle(frame_filter, (rect[0], rect[1]), (rect[2], rect[3]), (255, 255, 0), -1)
            cv2.putText(frame_filter, name, (rect[0] + 10, rect[1] + 35), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 2)
    ...
    canvas[frame_y:frame_y + frame.shape[0], frame_x:frame_x + frame.shape[1]] = frame_filter
