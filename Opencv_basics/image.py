...
def select_image():
    # Create a new Tk root instance to open the file dialog
...
def process_and_display_images(original_img):
    ...
    while True:
        ...
        if choice == '1':
            ...
            img_processed_name = 'Thermal Frame'
            cv2.imshow(img_processed_name, img_processed)
            plt.imshow(cv2.cvtColor(img_processed, cv2.COLOR_BGR2RGB))
            plt.show()
        elif choice == '9':
...
if original_img is not None:
    process_and_display_images(original_img)
else:
